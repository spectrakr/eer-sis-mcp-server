    ...
    server.registerTool(
        "kb_get_translate_script_km_contents",
        {
            ...
            inputSchema: {
                ...
                nodeId: z.string().optional().describe("노드 ID (예: NODE0000000456)"),
                serviceType: z.string().optional().describe("서비스 타입 (기본값: SVKNW)"),
                includeContents: z.boolean().optional().describe("지식 내용(contents) 포함 여부 (기본값: true)"),
            },
        },
        async (args) => {
            const response = await callCommand<KbDetailResponse>("kbUIService.getTranslateScriptKmContents", {
                kbId: args.kbId,
                nodeId: args.nodeId ?? "",
            ...
            if (args.includeContents !== false) {
                summary.contents = sanitizeKbContents(kb.transScriptContents ?? kb.contents);
            }
