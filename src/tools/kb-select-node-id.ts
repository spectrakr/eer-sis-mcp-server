...
export function registerKbSelectNodeId(server: McpServer): void {
    server.registerTool(
        "kb_select_node_id",
        {
            ...
            inputSchema: {
                alias: z.string().describe("고객 ID (티켓의 customerId 값, 예: 49)"),
                customerNo: z.string().describe("고객사 명 (티켓의 customerNo 값, 예: LGU+)"),
                moreFlag: z.boolean().optional().describe("추가 플래그 (기본값: false)"),
            },
        },
        async (args) => {
            const response = await callCommand<SelectNodeIdResponse>("kbUIService.selectNodeId", {
                alias: args.alias,
