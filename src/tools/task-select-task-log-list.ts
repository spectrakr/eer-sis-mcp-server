...
export function registerTaskSelectTaskLogList(server: McpServer): void {
    server.registerTool(
        ...
        async (args) => {
            ...
            try {
                const response = await callCommand<TaskLogListResponse>("taskUIService.selectTaskLogList", params);

                // 에러 체크
                if (response.ajaxCallResult !== "S" && response.processResult !== "S") {
                    return errorContent(response.ajaxCallMessage || "업무 로그 조회에 실패했습니다");
                }
                ...
                return errorContent(error instanceof Error ? error.message : "업무 로그 조회 중 오류 발생");
            }
        },
    );
}
